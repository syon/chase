...
async function coordinateCatalogLatest100() {
  console.log('[#coordinateCatalogLatest100] start')
  const pkt100Arr = await $cache.selectCachedPocketLatest100()
  const newArr = await Promise.all(
    pkt100Arr.map(async (item) => {
      // if Twitter
      const { eid } = item
      const { dig } = await $cache.digTable.get({ eid })
      return { ...item, ...dig }
    })
  )
