...
const debug = Debug('@:lambda-pocket')
...
async function get(accessToken, options) {
  debug('[#get]>>', options)
...
async function addTag(accessToken, itemId, tags) {
...
async function clearTags(accessToken, itemId) {
  debug('[clearTags]>>>>', accessToken, itemId)
  const q = new URLSearchParams()
  q.append('access_token', accessToken)
  q.append('item_id', itemId)
  const url = `${ENDPOINT}/pocket/send/tags/add?${q.toString()}`
