...
export default class CacheDB extends Dexie {
  ...
  async selectCachedPocketRecent() {
    dg('[#selectCachedPocketRecent] start')
  ...
  async putBulk(entries) {
    dg('[#putBulk]', Object.keys(entries).length)
  ...
  async putHatebuBulk() {
    dg('[#putHatebuBulk] >>')
    const latestEntries = await this.pocketTable
      .reverse()
      .limit(100)
      .sortBy('time_updated')
    ...
    dg('[#putHatebuBulk] length', targets.length)
  ...
  async getHatebuCntSet() {
    dg('[#getHatebuCntSet]')
    const arr = await this.hatebuTable.toArray()
